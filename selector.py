...
import MVO as mvo
import GWO as gwo
import RLGWO as rlgwo
import mGWO as mgwo
import mRLGWO as mrlgwo
import MFO as mfo
import CS as cs
import BAT as bat
import csv
...
def selector(algo,func_details,popSize,Iter,trainDataset,testDataset):
    ...
    if(algo==0):
        x=rlgwo.GWO( getattr(costNN, function_name),lb,ub,dim,popSize,Iter,trainInput,trainOutput,net)
    ...
    if(algo==2):
        x=mgwo.GWO( getattr(costNN, function_name),lb,ub,dim,popSize,Iter,trainInput,trainOutput,net)
        # x=pso.PSO( getattr(costNN, function_name),lb,ub,dim,popSize,Iter,trainInput,trainOutput,net)
        # x=gwo.GWO( getattr(costNN, function_name),lb,ub,dim,popSize,Iter,trainInput,trainOutput,net)
    if(algo==3):
